...
ImageDiff.createDiff = function (options, cb) {
  ...
  var diffCmd = 'compare';
  if (options.imageMagickPath)
    diffCmd = options.imageMagickPath + diffCmd;
...
ImageDiff.prototype = {
  rawDiff: function (options, callback) {
    ...
    var imageMagickPath = options.imageMagickPath;
...
function imageDiff(options, cb) {
  // import gm, pass through working folder if provided as arg.
  var gmOptions = {imageMagick: true};
  if (options.imageMagickPath)
    gmOptions.appPath = options.imageMagickPath;
  gm = require('gm').subClass(gmOptions);
